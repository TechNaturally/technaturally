...
/*
 * source: https://stackoverflow.com/a/38931547/1798697
 *
 */
...
/** 
 * Adapted from source: https://gist.github.com/Xeoncross/7663273
 *
 ...
 * @param null x
 */
function ajax(url, callback, data, config, x) {
  try {
    config = config || {};
    x = new(this.XMLHttpRequest || ActiveXObject)('MSXML2.XMLHTTP.3.0');
    x.open(config.method ? config.method : (data ? 'POST' : 'GET'), url, 1);
    x.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    x.setRequestHeader('Content-type', config.multipart ? 'multipart/form-data' : 'application/x-www-form-urlencoded');
    x.onreadystatechange = function () {
      x.readyState > 3 && callback && callback(x.responseText, x);
    };
    if (data.constructor.name == 'FormData' && !config.multipart) {
